...
var Token = exports.Token = Class.extend({
	...
	initialize: function (value, filename, pos, lineNo, column) {
		this._value = value;
	...
	serialize: function () {
		return [
			...
			this.pos,
			this._value
			// lineNo and column are optinal information
		];
	}
...
var Parser = exports.Parser = Class.extend({
	$isIdentifier: function (str) {
		var matched = _Lexer.rxIdent.exec(str);
		return matched != null && matched[0].length == str.length;
	},
	...
	_getColumn: function () {
		...
		return part.length - part.lastIndexOf("\n");
	},
	...
	_advanceToken: function () {
		...
		if(matched != null) {
			...
			this._lineNo += matched[0].split(_Lexer.rxNewline).length - 1;
		}
	},
		...
		for (var i = 0; i < expected.length; ++i) {
			if (this._input.substring(this._pos, this._pos + expected[i].length) == expected[i]) {
				if (expected[i].match(_Lexer.rxIdent) != null
					...
					return new Token(expected[i], this._filename, this._pos, this._lineNo, this._getColumn());
				}
			}
		}
		return null;
	...
	_expectIdentifierOpt: function () {
		...
		return new Token(matched[0], this._filename, this._pos, this._lineNo, this._getColumn());
	},
	...
	_qualifiedName: function () {
		...
		return new Token(name, firstToken.filename, firstToken.pos, firstToken.lineNo, firstToken.column);
	},
	...
	_memberDefinition: function (classFlags) {
		...
		if (! this._expect(";"))
			return null;
		return new MemberVariableDefinition(name, name.getValue(), flags, type, initialValue);
	},
	...
	_functionDefinition: function (token, flags, classFlags) {
		...
		if ((flags & ClassDefinition.IS_NATIVE) != 0 || (classFlags & ClassDefinition.IS_INTERFACE) != 0) {
			if (this._expect(";") == null)
				return null;
			return new MemberFunctionDefinition(name, name.getValue(), flags, returnType, args, null, null, null);
