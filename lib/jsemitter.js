...
var JavaScriptEmitter = exports.JavaScriptEmitter = Class.extend({
	...
	initialize: function (outputFile, platform) {
		this._output = platform.load("lib/js/bootstrap.js") + "\n";
		if(outputFile) {
			// source map is optional
			this._sourceMapGen = platform.createSourceMapGenerator(outputFile);
		}
		this._indent = 0;
	...
	_emitFunctionBody: function (funcDef) {
		try {
			...
			for (var i = 0; i < locals.length; ++i) {
				...
				this._emit("var " + name.getValue() + " = ", name);
	...
	_emit: function (str, token) {
		...
		if(this._sourceMapGen != null && token != null) {
			...
			var tokenValue = Parser.isIdentifier(token.getValue())
				? token.getValue()
				: null;
